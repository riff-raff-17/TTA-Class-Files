...
// resultCode: 'G' (green), 'Y' (yellow), '_' (gray)
void printColoredTile(char letter, char resultCode) {
    ...
    if (resultCode == 'G') {
        ...
        // Gray/white background, black text
...
int main() {
    ...
    cout << "===== WORDLE (C++ Console Version) =====\n";
    cout << "Guess the " << WORD_LENGTH << "-letter word in " << MAX_TRIES << " tries!" << endl;
    ...
    for (int attempt = 1; attempt <= MAX_TRIES; ++attempt) {
        ...
        if ((int)guess.size() != WORD_LENGTH) {
            cout << "Please enter exactly " << WORD_LENGTH << " letters." << endl << endl;
        ...
        for (int i = 0; i < WORD_LENGTH; ++i) {
            ...
            if (feedback[i] == 'G') {
                ...
                if (!green[idx]) { // don't downgrade a green to yellow
                    yellow[idx] = true;
                }
            }
        }
