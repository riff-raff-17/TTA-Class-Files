...
# How many pieces of food should be on screen at once.
NUM_FOOD = 3
...
def spawn_food_items(count, occupied_cells):
    """Spawn `count` food cells, none overlapping the snake or each other."""
    foods = []
...
score = 0
# Multiple food items now live in a list instead of a single tuple.
food_items = spawn_food_items(NUM_FOOD, snake)
...
while running:
    ...
    if not game_over:
        ...
        if move_timer >= MOVE_INTERVAL:
            ...
            if new_head in food_items:
                score += 1
                food_items.remove(new_head)
                # Replace the eaten food so there are always NUM_FOOD on screen.
                new_food = random_empty_cell(set(snake) | set(food_items))
